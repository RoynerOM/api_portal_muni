...
$uploadFileDir = $_SERVER['DOCUMENT_ROOT'] . '/documentos/acuerdos/';

if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    $stmt = $pdo->query("SELECT * FROM Acuerdos;");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_FILES['url']) && $_FILES['url']['error'] == 0) {
        ...
        if (in_array($doc_type, $allowed_types)) {
            ...
            $doc_url = 'https://muniupala.go.cr/documentos/acuerdos/' . $doc_name;
            ...
            if (move_uploaded_file($doc_temp, $doc_path)) {
                ...
                $actaId = isset($_POST['actaId']) ? $_POST['actaId'] : '';

                // Consulta SQL
                $sql = "INSERT INTO Acuerdos (year,actaId, fecha, url, nombre) VALUES (:year,:actaId, :fecha, :url, :nombre)";
                $stmt = $pdo->prepare($sql);

                // Asignación de parámetros
                $stmt->bindParam(':actaId', $actaId);
...
if ($_SERVER['REQUEST_METHOD'] === 'DELETE') {
    ...
    if ($id) {
        ...
        $sql = "SELECT url FROM Acuerdos WHERE id = :id";
        $stmt = $pdo->prepare($sql);
        ...
        if ($result) {
            ...
            $stmt->bindParam(':id', $id);
            if ($stmt->execute()) {
                echo json_encode(['status' => 'success', 'message' => 'Acuerdo eliminado correctamente.']);
            } else {
                echo json_encode(['status' => 'error', 'message' => 'Error al eliminar el acuerdo en la base de datos.']);
            }
        } else {
            echo json_encode(['status' => 'error', 'message' => 'Acuerdo  no encontrado.']);
        }
    } else {
