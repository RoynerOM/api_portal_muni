...
$uploadFileDir = $_SERVER['DOCUMENT_ROOT'] . '/documentos/plan_institucional/';

if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    $stmt = $pdo->query("SELECT * FROM Plan_Institucional;");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_FILES['url']) && $_FILES['url']['error'] == 0) {
        ...
        if (in_array($doc_type, $allowed_types)) {
            ...
            $doc_url = 'https://muniupala.go.cr/documentos/plan_institucional/' . $doc_name;
...
if ($_SERVER['REQUEST_METHOD'] === 'DELETE') {
    ...
    if ($id) {
        ...
        $sql = "SELECT url FROM Plan_Institucional WHERE id = :id";
        $stmt = $pdo->prepare($sql);
        ...
        if ($result) {
            ...
            $sql = "DELETE FROM Plan_Institucional WHERE id = :id";
            $stmt = $pdo->prepare($sql);
            $stmt->bindParam(':id', $id);
            if ($stmt->execute()) {
